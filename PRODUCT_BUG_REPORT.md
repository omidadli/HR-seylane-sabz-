...
- **Honest AI surfaces:** fallback scoring labeled `aiAvailable:false` with a visible "local engine (no Gemini)" disclaimer; empty resume → 400 (no 7.0); bulk upload requires real files → candidates created UNSCORED (no RNG, no auto-reject); chat keeps conversation memory and executes declared tools; rejection drafts only for a named candidate; radar/compare emit `null` gaps instead of fabricated 0s (REC-01/02/05/06/07, AIA-02/03/04, D4).
